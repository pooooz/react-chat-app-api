...
const task1 = async () => {
  ...
  try {
    const { stdout, stderr } = await execute(`node ./lesson-1/src/primeNumbers/index.js --a ${num1} --b ${num2}`);
    if (!stderr) {
      console.log(stdout);
    }
  } catch (error) {
...
const task2 = async () => {
  ...
  while (!file) {
    ...
    try {
      const { stdout, stderr } = await execute(`node ./lesson-1/src/headsOrTails/index.js --stat ${file} --guess ${guess}`);
      if (!stderr) {
        console.log(stdout);
      }
    } catch (error) {
